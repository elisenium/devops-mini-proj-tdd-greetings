...
function greet(name, language) {
    selectedLanguage =
        languages.find((item) => item.lang === language) || defaultLanguage; //If there is a language set in parameter, find it. Else, set default language
    ...
    if (name.length === 1 || typeof name === 'string') {
        ...
        if (containsUppercaseOnly(name) === true) {
            //If the name received is in uppercase, put everything in uppercase
    ...
    let andHello =
        `${selectedLanguage.and} ${selectedLanguage.greeting}`.toUpperCase(); //"AND HELLO" in chosen language
...
function moreThanTwo(namesLower, language) {
    ...
    return `${selectedLanguage.greeting} ${
        rest.length > 0 ? rest + ', ' : ''
    }${lastTwo}.`;
}
